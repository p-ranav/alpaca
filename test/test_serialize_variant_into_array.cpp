...
TEST_CASE("Serialize variant into array" * test_suite("variant")) {
  struct my_struct {
...
TEST_CASE("Serialize map<string, variant> into array" * test_suite("variant")) {
  struct my_struct {
...

  {
    ...
    if constexpr (detail::is_system_little_endian()) {
      ...
      REQUIRE(bytes[43] == static_cast<uint8_t>(0x1f)); // port: 8080
      REQUIRE(bytes[44] == static_cast<uint8_t>(0x90));
    }
