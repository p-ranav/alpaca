...
TEST_CASE("Deserialize int with crc32" * test_suite("crc32")) {
  ...
  {
    std::error_code ec;
    auto result = deserialize<my_struct, std::vector<uint8_t>, options::with_checksum>(bytes, ec);
...
TEST_CASE("Deserialize int with crc32 (error)" * test_suite("crc32")) {
  ...
  // deserialize
  {
    ...
    deserialize<my_struct, std::vector<uint8_t>, options::with_checksum>(bytes, ec);
