...
TEST_CASE("Deserialize crc32 error - invalid_argument" * test_suite("crc32")) {
  ...
  // deserialize
  {
    ...
    std::error_code ec;
    deserialize<my_struct, std::array<uint8_t, 2>, options::with_checksum>(received, ec);
...
TEST_CASE("Deserialize crc32 error - bad_message" * test_suite("crc32")) {
  ...
  // deserialize
  {
    ...
    std::error_code ec;
    deserialize<my_struct, std::array<uint8_t, 5>, options::with_checksum>(received, ec);
    REQUIRE((bool)ec == true);
